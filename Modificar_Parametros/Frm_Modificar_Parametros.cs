...
{
    public partial class Frm_Modificar_Parametros : Form
    {
        #region Variables
        ...
        #endregion
        public Frm_Modificar_Parametros()
        {
            InitializeComponent();
        }

        private void Frm_Modificar_Parametros_Load(object sender, EventArgs e)
        {
            conectar();
            Cargar_dtmensajes();
            buscar_mensajes();
        ...
        protected void agregarimagen()
        {
            foreach (DataGridViewRow row in grilla_parametros.Rows)
            {
                ...
                DataGridViewImageCell Imagen = row.Cells["Vigente"] as DataGridViewImageCell;

                if (ve == "S")
                {
                    Imagen.Value = (System.Drawing.Image)Modificar_Parametros.Properties.Resources.Check;
                }
                else
                {
                    Imagen.Value = (System.Drawing.Image)Modificar_Parametros.Properties.Resources.Delete;
        ...
        private void grilla_parametros_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == 1)
                {
                    codigo = Convert.ToInt32(grilla_parametros.Rows[e.RowIndex].Cells["Codigo2"].Value.ToString());
                    txtdescripcion.Text = grilla_parametros.Rows[e.RowIndex].Cells["Descripcion2"].Value.ToString().ToUpper();
                    vigencia = grilla_parametros.Rows[e.RowIndex].Cells["V"].Value.ToString().ToUpper();

                    DialogResult opc = MessageBox.Show("Estimado usuario, Desea Modificar Vigencia ?", "Informacion", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (opc == DialogResult.Yes)
                    {
                        foreach (DataRow fila in dt.Select(" CODIGO= " + codigo))
                        {
